...
class sfss_get_count_from_hive_day:
    ...
    def get_all(self):
        ...
        for user_info in user_info_list:
            ...
            # )
            sql = "replace into customer_call_record_month (id_type, id_num, uid, record_date, hit_info, all_count) VALUES " \
                  "(%s,%s,%s,%s,%s,%s)" % (
...
def get_1st_of_last_month(today_str):
    """
    获取上个月第一天的日期
    :return: 返回日期
    """
    today = string_toDatetime(today_str)
    year = today.year
    month = today.month
    if month == 1:
        month = 12
